...
import net.nightpool.bukkit.listener.PlayerListener;
...
public class RejewledPlugin extends JavaPlugin {
    public HashSet<Integer> hiddenBlocks;
    ...
    public void onEnable() {
        ...
        for (Integer i : getConfig().getIntegerList("hidden_blocks")) {
            Material m = Material.getMaterial(i);
            if (m == null) {
                getLogger().warning("Material " + m + " in hidden_blocks is not a valid material.");
            } else {
                hiddenBlocks.add(m.getId());
            }
        }
        getLogger().info("Hidden Blocks: "+hiddenBlocks.toString());
    ...
    public void hook(Player player) {
        ...
        JeweledConnection handlerHook = new JeweledConnection(this, server.getServer(),
